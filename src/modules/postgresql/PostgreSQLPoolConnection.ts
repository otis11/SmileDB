...
export class PostgreSQLPoolConnection implements PoolConnection {
    ...
    async fetchTables() {
        const timer = new Timer();
        let queryResult = await this.pool.query(`SELECT table_name
    ...
    private createQueryResultField(field: FieldDef, fieldConstraintsHashMap: any): QueryResultField {
    ...
    private createQueryResultFields(fields: FieldDef[], fieldConstraintsHashMap: any) {
...
${whereStatements}`);
    ...
    convertJavascriptValueToPSQL(val: any) {
        if (val === null) {
            return 'NULL';
        }
    ...
    async executeQueriesAndFetch(queries: string[], config: QueryConfigFetch) {
        ...
        await pool.end();
        const fieldConstraintsHashMap: any = {};
