import { webviewCopyToClipboard, webviewGetConnectionConfig, webviewLogError, webviewSelectTextInContentEditableDiv } from "../webview-internals"
...
function onSelectionModeDoubleClick(e: MouseEvent) {
    ...
    colElement?.focus()
    webviewSelectTextInContentEditableDiv(colElement)
    selectionModeClearSelectedCols()
}
...
export function copySelectedColumns() {
    ...
    if (selection) {
        webviewCopyToClipboard(selection)
        return
    }
    ...
    webviewCopyToClipboard(content.slice(0, -1))
}
...
export function renderSelectionModeContextMenu() {
    ...
    tableSelectionContextMenuSelectRowElement.addEventListener('click', () => {
        ...
        if (!row) {
            webviewLogError('Error selecting row (row not found)', row)
            return
        }
    ...
    tableSelectionContextMenuSelectColumnElement.addEventListener('click', () => {
        if (!tableElement) {
            webviewLogError('Error selecting all (tableElement not found)', tableElement)
            return
        }
