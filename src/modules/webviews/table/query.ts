import { QueryResult, QueryResultField, QueryResultRow } from "../../../shared/types"
...
function getFieldsToIdentifyUpdate(): QueryResultField[] {
    ...
    const fields = queryResult.fields.flatMap(field => {
...
function createQueryPreviewElement(title: string, queries: string[]) {
    ...
    const queriesTitle = '<div class="queries-preview-title">-- ' + webviewHtmlSanitize(title) + '</div>'
    let preString = '<pre>'
    for (let i = 0; i < queries.length; i++) {
        const element = queries[i]
        preString += webviewHtmlSanitize(element) + '\n'
    }
    preString += '</pre>'
