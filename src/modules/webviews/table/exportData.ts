...
import { webviewCopyToClipboard, webviewGetConnectionConfig, webviewLogWarn, webviewVscode } from "../webview-internals"
...
export function updateExportDataFolderLocation(folderPath: string | null) {
    if (folderPath !== null) {
        const conn = webviewGetConnectionConfig()
...
function createExportDataString(rowCount: number) {
    ...
    if (exportDataFormat === 'CSV') {
        ...
        webviewLogWarn('exportDataPreview unknown format', exportDataFormat)
        return ''
    }
}
...
function addExportDataEventListeners() {
    ...
    document.getElementById('export-copy-to-clipboard')?.addEventListener('click', () => {
        webviewCopyToClipboard(createExportDataString(Number.MAX_SAFE_INTEGER))
    })
