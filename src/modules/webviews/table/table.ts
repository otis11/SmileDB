import { QueryResultRow } from "../../../shared/types"
import { webviewCreateCodiconElement, webviewGetConnectionConfig, webviewHtmlSanitize, webviewLogDebug } from "../webview-internals"
...
export async function renderTableResult() {
    ...
    requestAnimationFrame(() => {
        webviewLogDebug("Table render took: " + (performance.now() - startRenderTable).toFixed(2) + ' ms', queryResult)
...
function renderTableHeader() {
    ...
    for (let i = 0; i < l; i++) {
        ...
        iconContainerOrderByUp.appendChild(webviewCreateCodiconElement('triangle-up'))
        iconContainerOrderByDown.appendChild(webviewCreateCodiconElement('triangle-down'))
        iconContainerOrderByDefault.appendChild(webviewCreateCodiconElement('triangle-up', true))
        iconContainerOrderByDefault.appendChild(webviewCreateCodiconElement('triangle-down', true))
...
export function createTableRow(i: number, row: QueryResultRow, isNewRow = false) {
    ...
    for (let j = 0; j < Object.keys(row).length; j++) {
        ...
        cellElementString += '>'
        if (row[fieldName] !== null) {
            cellElementString += webviewHtmlSanitize(row[fieldName]?.toString())
        }
