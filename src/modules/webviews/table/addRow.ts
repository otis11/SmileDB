...
import { webviewCreateElementFromHTMLString } from '../webview-internals'
...
export function renderAddRow() {
    addElement?.addEventListener('click', async () => {
        ...
        if (tableElement) {
            ...
            tableElement?.insertBefore(webviewCreateElementFromHTMLString(rowHtmlString), tableOverlay)
