...
import { SQLDatabaseStats, SQLPoolConnection } from "../../shared/types"
import { DatabaseTreeItem } from "./DatabaseTreeItem"
import { SchemaTreeItem } from "./SchemaTreeItem"
import { showMessage } from "../../shared/helper"
import { FolderTreeItem } from "./FolderTreeItem"
import { TableTreeItem } from "./TableTreeItem"
import { ProcedureTreeItem } from "./ProcedureTreeItem"
...
export async function getDatabaseTreeChildrenSQL(item: TreeItem, config = { schemas: false }): Promise<TreeItem[]> {
    if (item instanceof PoolConnectionTreeItem) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (key === 'tables') {
                if (getExtensionConfig().tree.expandTablesInstantly && result[key] > 0) {
